...
public class Game {
    ...
    public Game(int width, int height,Level level) {
        ...
        this.ball = new Ball(20,20,new Point2D(this.player.getPaddle().getPosition().getX(),this.player.getPaddle().getPosition().getY()+25),new Point2D(100,100), this);
        this.brickCount = level.getBrickCount();
        this.brickMap = new BrickMap(level,this);
        this.hearts = new ArrayList<>();
        for(int i=0; i < this.player.getLives();i++){
            this.hearts.add(new Image("C:\\Users\\Štěpán\\Desktop\\java-projekt\\BreakOut-cd5fd7ee5b5cbd8bbf3662b1a98c735e409d5345\\Projekt-BreakOut\\src\\main\\resources\\Images\\heart.png"));
        }
    }
